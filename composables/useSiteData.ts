  ...
  watch(
    ...
    (val) => {
      if (val) {
        ...
        // Only overwrite with API data when we have items, so we never replace with empty (e.g. if API fails or returns [])
        if (items.length > 0) {
          siteData.announcements = { title: val.title ?? siteData.announcements.title, items }
        } else {
          siteData.announcements.title = val.title ?? siteData.announcements.title
        }
      }
    },
    { immediate: true }
